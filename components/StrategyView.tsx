          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              ...
              <button 
                ...
                disabled={isSaving} 
                className={`px-4 md:px-6 py-1.5 md:py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-1 md:gap-2 transition-all shadow-md ${isCurrentTabDirty ? 'bg-brand-600 text-white hover:bg-brand-700 shadow-brand-100' : 'bg-slate-100 text-slate-400 cursor-default'}`}
              >
                {isSaving ? <Loader2 className="animate-spin h-3 w-3"/> : <Save size={14} />} 
                {isCurrentTabDirty ? saveButtonLabel : '已是最新'}
              </button>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-5xl mx-auto space-y-4 md:space-y-6 pb-12 py-4 md:py-6">
            <div className="flex flex-col items-center gap-4 mb-6 md:mb-10">
               <div className="flex gap-1 bg-white p-1 rounded-2xl border shadow-sm w-full md:w-auto overflow-x-auto">
                  ...
                  <button onClick={() => setDeptViewMode('quarterly')} className={`flex-1 md:flex-none px-4 md:px-8 py-2 rounded-xl text-[10px] md:text-[11px] font-black transition-all uppercase tracking-widest whitespace-nowrap ${deptViewMode === 'quarterly' ? 'bg-slate-900 text-white shadow-lg' : 'text-slate-400 hover:text-slate-600'}`}>部门季度目标</button>
               </div>
               {deptViewMode === 'quarterly' && (
