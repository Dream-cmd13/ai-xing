    ...
    <div className="h-full flex flex-col gap-4 overflow-hidden relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-white p-3 rounded-2xl shadow-sm border border-slate-200 shrink-0 gap-3 md:gap-0">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
           ...
           <button onClick={onSaveProcesses} disabled={isSaving || Boolean(backendError) || !canEditCurrentProcess} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2 transition-all ${isDirty ? 'bg-brand-600 text-white hover:bg-brand-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'} ${(backendError || !canEditCurrentProcess) ? 'opacity-50 cursor-not-allowed' : ''}`}>
