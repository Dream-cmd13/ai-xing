-- Preserve the legacy department OKR RPC contract while keeping `periods`
-- object-shaped after task attachment. The previous implementation used an
-- array aggregation for period entries, which produced [{"Q3": [...]}] instead
-- of {"Q3": [...]} and made Node clients treat every period as empty.
BEGIN;
  ...
  IF p_department_id IS NOT NULL AND btrim(p_department_id) <> '' THEN
    ...
    LOOP
      v_node := jsonb_build_object(
        'id', v_row.id,
        'name', v_row.name,
        'managerName', v_row.manager_name,
  ...
  IF COALESCE(p_include_tasks, TRUE) THEN
    SELECT COALESCE(jsonb_agg(
      item || jsonb_build_object('periods', (
        SELECT COALESCE(
          jsonb_object_agg(period_key, public.mcp_attach_tasks_to_okrs(period_value, v_name_map)),
          '{}'::JSONB
        )
...
REVOKE ALL ON FUNCTION public.mcp_get_department_okrs(TEXT, INTEGER, TEXT, BOOLEAN, INTEGER)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.mcp_get_department_okrs(TEXT, INTEGER, TEXT, BOOLEAN, INTEGER)
  TO authenticated;
