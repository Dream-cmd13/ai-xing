-- Fix the OKR task attachment projection without rewriting business data.
-- The prior loop assigned a JSONB variable from a two-column record and then
-- read a non-existent "value" property, which emptied every OKR when
-- includeTasks=true. This migration keeps the existing bounded single-scan
-- behavior and only corrects the record-to-entry assignment.
BEGIN;
...
BEGIN
  ...
  -- Collect all slots: <okrId>-kr-<krIndex>.
  ...
  IF array_length(v_slots, 1) IS NOT NULL THEN
    -- A SECURITY DEFINER function must apply the task visibility predicate explicitly.
    FOR v_task_rows IN
  ...
  -- Read the entry column from the WITH ORDINALITY record. The previous
  -- implementation treated the record itself as JSONB and returned null.
  FOR v_okr_record IN
